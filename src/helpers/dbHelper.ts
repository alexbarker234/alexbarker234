...
export const getCurrentGame = async () => {
  ...
  return await collectionExt.findOneOrUpsert({ dateID: dateID }, { dateID: dateID, clicks: 0 });
};
...
class CollectionExtended<Document> {
  ...
  findOneOrUpsert = async function (filter: Filter<Document>, setOnUpsert: object) {
    const options = { upsert: true, returnNewDocument: true };
    const update = { $setOnInsert: setOnUpsert };
    let result = await this.collection.findOneAndUpdate(filter, update, options);
